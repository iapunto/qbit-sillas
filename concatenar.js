...
function leerArchivos(dir, fileList = []) {
  ...
  files.forEach((file) => {
    ...
    if (stats.isDirectory()) {
      if (
        ["node_modules", ".cspell", ".vscode", ".astro", "assets"].includes(
          file
        )
      ) {
...
function esArchivoRelevante(file) {
  const extensionesValidas = [".ts", ".json"];
...
const archivoSalida = "./archivos_proyecto.txt"; // Nombre del archivo de salida
